...
# ─── Constantes ───────────────────────────────────────────────
CAJON_A_KG           = 10
PRECIO_MINORISTA_KG  = 450   # valor por defecto si no hay archivo
PRECIO_MAYORISTA_KG  = 320   # valor por defecto si no hay archivo
STOCK_INICIAL        = 5000
...
class AjustePrecioSchema(BaseModel):
    precio_minorista_kg: float
    precio_mayorista_kg: float
...
def leer_precios() -> dict:
    if os.path.exists(ARCHIVO_PRECIOS):
        try:
            ...
            pass
    return {"precio_minorista_kg": PRECIO_MINORISTA_KG, "precio_mayorista_kg": PRECIO_MAYORISTA_KG}

def guardar_precios(minorista: float, mayorista: float):
    with open(ARCHIVO_PRECIOS, "w", encoding="utf-8") as f:
        json.dump({"precio_minorista_kg": minorista, "precio_mayorista_kg": mayorista}, f, indent=2)

def precio_por_kg(tipo_cliente: str) -> float:
    precios = leer_precios()
    return precios["precio_minorista_kg"] if tipo_cliente == "Minorista" else precios["precio_mayorista_kg"]
...
def raiz():
    return {
        ...
        "version": "3.0.0",
        "endpoints": ["/stock", "/precio", "/pedidos", "/pedidos/sync", "/reporte"]
    }
...
def ver_stock():
    ...
    return {
        ...
        "precio_minorista_kg": precios["precio_minorista_kg"],
        "precio_mayorista_kg": precios["precio_mayorista_kg"],
    }
...
# ══════════════════════════════════════════════════════════════
# PRECIOS — se actualizan desde la web de contabilidad
# ══════════════════════════════════════════════════════════════
...
@app.put("/precio")
def actualizar_precio(body: AjustePrecioSchema):
    if body.precio_minorista_kg <= 0 or body.precio_mayorista_kg <= 0:
        raise HTTPException(status_code=400, detail="Los precios deben ser mayores a 0")
    guardar_precios(body.precio_minorista_kg, body.precio_mayorista_kg)
    return {
        "mensaje":             "Precios actualizados",
        "precio_minorista_kg": body.precio_minorista_kg,
        "precio_mayorista_kg": body.precio_mayorista_kg,
    }
